...
function showPage(name) {
  ...
  // 1) lancer la peinture qui couvre l'écran
  pageTransition.classList.add('active');

  // 2) quand l'animation est finie (0.5s), changer de page
  setTimeout(() => {
    ...
    // 3) une fois la page changée, on enlève la peinture
    // (elle redescend grâce au CSS)
    setTimeout(() => {
      pageTransition.classList.remove('active');
    }, 50); // petit délai pour être sûr que la nouvelle page est peinte

  }, 500); // durée EXACTE de transition CSS (.5s)
}
  ...
  function openLightbox(productName, materials, price, description, options = {}) {
    ...
    currentProduct = { id: productId, name: productName, materials, price };
    ...
    newAddBtn.addEventListener('click', function () {
      ...
      const personalization = customEl.value.trim();
      const cart = getCart();
  ...
  function openLightboxFromCart(index) {
    ...
    // retrouver éventuellement les images depuis products (optionnel)
  ...
  function createProductCard(product) {
    ...
    article.addEventListener('click', () => {
      openLightbox(
        ...
        product.price,
        product.description,
        { images: product.images, productId: product.id }
      );
    });
